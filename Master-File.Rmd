...
```{r merge positive friends counts}
...
```{r corr matrix}
d_cor <- dplyr::select(main1, pos_indegree, pos_outdegree, neg_indegree, neg_outdegree, n_mutual_bfs, non_reciprocal) 
cor(d_cor)
...
### Other measures of school satisfaction
w1: y1_asch1
How much do you agree with "School is not for ppl like me?" 1 = strongly agree, 5 = strongly disagree, 
w1: y1_tenc1 
I get the help that I need from the teachers at school
w1: y1_tenc2
My teachers encourage me at school
w1: y1_tenc3
There are teachers who threat me unfairly

```{r}
# recoding problem behavioral measures
main2 <- main2 %>% 
  mutate_at(vars(y1_tenc1:y1_tenc2), ~ 5-.) %>% 
  mutate(teacher_help = y1_tenc1,
         teacher_encouragement = y1_tenc2,
         teacher_support = y1_tenc1 + y1_tenc2)
# check before recoding
print("Before recoding")
descr::freq(main1$y1_tenc1, plot = FALSE)
print("After recoding")
descr::freq(main2$y1_tenc1, plot = FALSE)
print("Frequency of teacher support")
descr::freq(main2$teacher_support)
```
...
mydata.cor = cor(na.omit(main2[,c("y1_sat2","teacher_support","problem_behaviors","victim_history","delinquency_count")]), method = c("spearman"))
mydata.cor
corrplot(mydata.cor, method = "color")
```
...
```{r normality test}
shapiro.test(main2$y1_sat2) # p-value > 0.05 implying that the distribution of the data are not significantly different from normal distribution. In other words, we can assume the normality.
```

## Model1
```{r model1}
main2$classid.f <- as.factor(main2$classidCUF)
summary(lm(y1_sat2 ~ pos_indegree + pos_outdegree + pos_isolate + neg_indegree + neg_outdegree + neg_isolate + n_mutual_bfs  + non_reciprocal + (1|schoolidCUF) + age + as.factor(gender) + language + nationality + schooltype + problem_behaviors + as.factor(delinquency_history) + delinquency_count + as.factor(victim_history) + parent_relation + teacher_support + classid.f, data=main2))
...
## Model2
```{r model2}
main2$schoolid.f = as.factor(main2$schoolidCUF)
main2$classid.f <- as.factor(main2$classidCUF)
model1 = lm(y1_sat2 ~ pos_indegree + pos_outdegree + pos_isolate + neg_indegree + neg_outdegree + neg_isolate + (1|schoolidCUF) + age + as.factor(gender) + language + nationality + schooltype + y1_fact + classid.f, data=main2)
summary(model1)
...
```{r glmmPQL} 
library(MASS)
PQL <- glmmPQL(y1_sat2 ~ pos_indegree + pos_outdegree + pos_isolate + neg_indegree + neg_outdegree + neg_isolate + n_mutual_bfs  + non_reciprocal  + as.factor(gender) + language + nationality + schooltype + y1_fact, random = ~1 | schoolidCUF/classidCUF, family = gaussian(link = "log"),
    data = main2, verbose = FALSE)
summary(PQL)
```

# Friendship network descriptive statistics
```{r}
stats1 = main2 %>% group_by(schoolid.f) %>% 
  summarise(avg_pos_indegree = mean(pos_indegree),
            avg_pos_outdegree = mean(pos_outdegree),
            #avg_pos_isolate = mean(pos_isolate),
            avg_neg_indegree = mean(neg_indegree),
            avg_neg_outdegree = mean(neg_outdegree),
            #avg_neg_isolate = mean(neg_isolate),
            avg_n_mutual = mean(n_mutual_bfs),
            avg_non_reciprocal = mean(non_reciprocal))

summary(stats1[,-1])
#calculate descriptive statistics for 'x' and 'z' only
sapply(stats1[,-1], sd, na.rm=TRUE)
```

```{r}
stats2 = main2 %>% group_by(classid.f) %>% 
  summarise(avg_pos_indegree = mean(pos_indegree),
            avg_pos_outdegree = mean(pos_outdegree),
            #avg_pos_isolate = mean(pos_isolate),
            avg_neg_indegree = mean(neg_indegree),
