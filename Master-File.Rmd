...
main1 |> 
  dplyr::select(y1_sat2, neg_indegree, neg_outdegree, pos_indegree, pos_outdegree, n_mutual_bfs, non_reciprocal) |>  
...
```{r problem behaviors, eval=FALSE}
...
```{r delinquency behavior, eval=FALSE}
main2 <- main2 %>% 
...
w1: y1_asch1 How much do you agree with "School is not for ppl like me?" 1 = strongly agree, 5 = strongly disagree, w1: y1_tenc1 I get the help that I need from the teachers at school w1: y1_tenc2 My teachers encourage me at school w1: y1_tenc3 There are teachers who threat me unfairly
...
# create a Q-Q plot
qqnorm(main2$y1_sat2)
qqline(main2$y1_sat2)
hist(main2$y1_sat2)
```
...
main2$classid.f <- as.factor(main2$classidCUF)
model1 = lm(y1_sat2 ~ pos_indegree + pos_outdegree + pos_isolate + neg_indegree + neg_outdegree + neg_isolate + (1|schoolidCUF) + age + as.factor(gender) + language + nationality + schooltype + y1_fact, data=main2)
summary(model1)
...
```{r, eval=FALSE}
stargazer::stargazer(model1, style = "all", type = "latex")
```
