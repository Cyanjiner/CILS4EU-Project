...
# Now, Create Network Measures for all data in Wave I
...
Calculate other measures of reciprocity (Followers and non-reciprocal friends:
...
```{r}
#merge all these measures into one df: w1_reciprocal_yc
```
...
pos_netm$youthid <- as.numeric(paste0(netm$classid_chr, netm$youthid_chr))
```
...
w1_netm <- merge(neg_netm, pos_netm, by = "youthidCUF")

#now merge centrality measures w reciprocity measures
w1_netm <- merge(w1_netm, w1_reciprocal_yc)
```
...
```{r}
# df$classid.f <- as.factor(df$classidCUF)
# summary(lm(satisfaction ~ pos_indegree + pos_outdegree + pos_isolate + neg_indegree + neg_outdegree + neg_isolate + n_mutual_bfs  +  .... + [covariates here] + as.factor(y1_nationRV) + as.factor(schtype_geRV) + as.factor(y1_sex), + classid.f, data=df))
